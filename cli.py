...
def save_all(access_token, site_id, start_date, end_date, start_date2, end_date2, path, delay):
	time.sleep(delay)
...
@click.option('--delay', '-D', default=15, help='Delay in seconds between each request')
def fetch(access_token, path, date, delay):
	...
	for site in baidutongji.getSiteList(access_token)['list']:
		...
		save_all(access_token, site_id, date, date, yesterday, yesterday, day_path, delay)
		...
		if cron_week:
			...
			time.sleep(delay)
			save_all(access_token, site_id, this_week_start, this_week_end, last_week_start, last_week_end, week_path, delay)
		...
		if cron_month:
			...
			time.sleep(delay)
			save_all(access_token, site_id, this_month_start, this_month_end, last_month_start, last_month_end, month_path, delay)
...
@click.option('--delay', '-D', default=15, help='Delay in seconds between each request')
def fetch_all(ctx, access_token, path, start_date: click.DateTime, end_date: click.DateTime, delay: int):
	'''Fetch all history reports by indicating start date and end date in form of YYYY-MM-DD'''
	while start_date <= end_date:
		ctx.invoke(fetch, access_token=access_token, path=path, date=start_date.strftime('%Y-%m-%d'), delay=delay)
